<template>
	<div id="myContainer">
	    <div class="card">
		    <h3 class="card-header text-white bg-primary no-margin">Add Ayat Jurnal Penyesuaian <icon name="map"></icon></h3>
			<b-breadcrumb :items="items"/>
    		<div class="card-body">
                <b-row class="my-1">
				    <b-col sm="4"><label for="input-small">No. AJP:</label></b-col>
					<b-col sm="8">
				...
				<b-row class="my-1">
					...
					<b-col sm="8" v-if="selected > '0'">
						<div class="card">
							<h5 class="card-header text-white bg-primary no-margin">Jurnal</icon></h5>
							<div class="card-body">
								<div class="table-responsive">
									<table class="table">
										<thead>
										...
										<tbody>
											<tr v-for="d_akun in pass_data" v-bind:value="d_akun.id" :key="d_akun.id">
												...
												<td>
													<input v-model="d_akun.jumlah_debet" @change="totals()" type="number" class="form-control" placeholder="Ajp Debet">

												<td> <input type="number" class="form-control" @change="totals()" v-model="d_akun.jumlah_kredit" placeholder="Ajp Kredit"></td>
											</tr>
										</tbody>
									</table>
							</div>
		...
		data(){
			return{
				...
				items: [{
					...
					to: '/ayat-jurnal-penyesuaian'
				},{
					text: 'Add Ayat Jurnal Penyesuaian',
					href: '#'
				}]
		...
		methods:{
			...
			processForm: function() {
					...
					.then((response) => {
						...
						this.$router.push({name: 'Ajp', params: {id: this.saleid}});
					  });
